...
void wczytaj_liste_plik(Node** glowa) {
    ...
    while (fscanf(plik, "%100s %d %lf %40s %d %d" , gatunek,&dieta_int,&masa,zagroda,&temperament_int,&status_bezpieczenstwa_int) == 6) {
        Dino dino ;
...
void zapisz_liste(Node* glowa) {
    ...
    Node *obecny = glowa;
    while (obecny != NULL) {
        fprintf(plik, "%s %d %lf %s %d %d \n" , obecny->dane.gatunek,obecny->dane.dieta,obecny->dane.masa,obecny->dane.zagroda,obecny->dane.temperament,obecny->dane.status_bezpieczenstwa);
